...
핵심 법령:
  법인세법§28①4의2 (지급이자 손금불산입 — 차입금 과다 적수×60% 한도)
  법인세법§52 (특수관계 차입 부당행위계산 부인)
  소득세법§127 (원천징수 의무 — 이자소득 지급 시)
  상증§41 (특수관계인 이익분여 — DES 출자전환 시 증여 의제)
  조특§7 (중소기업 차입금 이자 세액공제)
  국기법§47의4 (이자상당액 가산세 — 차입금 관련 납부 불이행)
  외감법§4 (외부감사 대상 — 부채총액 70억 기준)
  K-IFRS 1107 (금융상품 공시 — 부채 리스크 정보)
  상법§469 (사채 발행 — 이사회 결의 절차)
  상법§464의2 (이익배당 한도 — 자본잠식 방지)
"""
...
class DebtStructureAgent(ProfessionalSolutionAgent):
    ...
    법인세법§28①4의2 차입금 과다 이자 손금불산입(적수×60% 한도)
    법인세법§52 특수관계 차입 부당행위계산 부인
    소득세법§127 이자소득 원천징수 의무
    조특§7 중소기업 차입금 이자 세액공제
    국기법§47의4 이자상당액 가산세 — 납부 불이행 시
    외감법§4 부채총액 기준 외감 대상 판정
    K-IFRS 1107 금융상품 공시 — 유동성·신용·시장 리스크
    상증§41 특수관계인 이익분여 — DES 출자전환 증여 의제 주의
    상법§464의2 이익배당 한도 — 자본잠식 방지
    """
