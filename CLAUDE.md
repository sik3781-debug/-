...
## 전문 솔루션 그룹 (Professional Solution Group)

에이전트 시스템의 모든 컨설팅 에이전트는 **전문 솔루션 그룹** 멤버로 분류됩니다.
에이전트 레지스트리: `agents/groups/professional_solution_group.py`
memory/ 표준 경로: `outputs/`, `memory/`, `discovery/`, `logs/`

핵심 법령 약식 인용 기준: 법§55, 소§17, 상증§63, 조특§10, 상법§341
자가검증 3축: ① 계산 정확성 ② 법령 정확성 ③ 4자관점(법인·주주·과세관청·금융기관) 누락 없음
